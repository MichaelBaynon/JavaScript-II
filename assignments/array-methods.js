...
let fullName = runners.map(function (banana) {
    return banana.first_name + ', ' + banana.last_name
});
...
let allCaps = runners.map(function (banana) {
    return banana.first_name.toUpperCase()
...
let largeShirts = runners.filter(function (banana) {
...
let ticketPriceTotal = runners.reduce(function (total, banana) {
    return total + banana.donation
}, 0);
...
// Problem 1            They changed the location and now we must email all the participants

let emailName = runners.map(function (banana) {
    return banana.email
});
console.log(emailName);

// Problem 2             All the shirts magically shrunk and now we need to remake all the shirts

let shirtSize = runners.map(function (banana) {
    return banana.shirt_size
});
console.log(shirtSize);

// Problem 3             The companies came together and agreed capitalizing their names on their posters would be benificial for  their buiseness now we must capatlize all their names

let capNames = runners.map(function (banana) {
    return banana.company_name.toUpperCase()
